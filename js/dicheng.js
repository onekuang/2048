...
function noBlockYou(row ,col1,col2,board){
	for(var i=col1-1;i>col2; i--)
		if(board[row][i]!=0)
			return false;
		return true;
...
function noBlockXia(jj,kk,ii,board){
	
	for( var i = kk+ 1 ; i < ii ; i ++ )
        if( board[jj][i] != 0 )
            return false;
    return true;
}
...
function noBlockUp(ii,kk,jj,board){
	for(var i=kk+1 ; i<ii; i++)
		if(board[i][jj]!=0)
			return false;
		return true;
